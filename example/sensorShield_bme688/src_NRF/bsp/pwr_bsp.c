...
int pwr_bsp_start() {
  ...
  // Power up sensors
  if (gpio_pin_set_dt(&gpio_SCD41_pwr, 1) < 0) {
  ...
  LOG_INF("Sensors powered up");

  // Connect I2C buses to sensors
  ...
  if (gpio_pin_set_dt(&gpio_I2C_SCD41_EN, 1) < 0) {
    LOG_ERR("SCD41 I2C EN GPIO configuration error");
    return -1;
  }
  LOG_INF("I2C buses connected to sensors");
