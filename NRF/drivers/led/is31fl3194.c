...
static const uint8_t led_channels[] = {IS31FL3194_OUT1_REG, IS31FL3194_OUT2_REG, IS31FL3194_OUT3_REG};
...
static const struct led_info *is31fl3194_led_to_info(const struct is31fl3194_config *config, uint32_t led) {
...
static int is31fl3194_get_info(const struct device *dev, uint32_t led, const struct led_info **info_out) {
...
static int is31fl3194_set_color(const struct device *dev, uint32_t led, uint8_t num_colors, const uint8_t *color) {
  const struct is31fl3194_config *config = dev->config;
  const struct led_info *info = is31fl3194_led_to_info(config, led);
  int ret;
  ...
  if (ret == 0) {
    ret = i2c_reg_write_byte_dt(&config->bus, IS31FL3194_UPDATE_REG, IS31FL3194_UPDATE_VAL);
  }
...
static int is31fl3194_check_config(const struct device *dev) {
  ...
  case 3:
    ...
    break;
  default:
    LOG_ERR("%s: invalid number of LEDs %d (must be 1 or 3)", dev->name, config->num_leds);
    return -EINVAL;
  }
...
static int is31fl3194_init(const struct device *dev) {
  ...
  if (prod_id != IS31FL3194_PROD_ID_VAL) {
    LOG_ERR("%s: invalid product ID 0x%02x (expected 0x%02x)", dev->name, prod_id, IS31FL3194_PROD_ID_VAL);
    return -ENODEV;
  }
...
#define IS31FL3194_DEFINE(id)                                                                                          \
  ...
  DT_INST_FOREACH_CHILD(id, COLOR_MAPPING)                                                                             \
                                                                                                                       \
  static const struct led_info is31fl3194_leds_##id[] = {DT_INST_FOREACH_CHILD(id, LED_INFO)};                         \
  static const uint8_t is31fl3194_currents_##id[] = {DT_INST_FOREACH_CHILD(id, LED_CURRENT)};                          \
  ...
  DEVICE_DT_INST_DEFINE(id, &is31fl3194_init, NULL, NULL, &is31fl3194_config_##id, POST_KERNEL,                        \
